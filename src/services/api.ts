...
class ApiServices {
	...
	async getAllBrandProducts(id: string | string[]): Promise<ProductResponse> {
	...
	async getBrand(id: string | string[]): Promise<SingleProductResponse> {
		return await fetch(
			`https://ecommerce.routemisr.com/api/v1/${id}`
		).then((res) => res.json());
	}
}
